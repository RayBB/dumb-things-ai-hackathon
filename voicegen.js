...
async function logQuotesWithoutAudio() {
  for (const category in techBroQuotes) {
    const quotes = techBroQuotes[category];
    for (let index = 0; index < quotes.length; index++) {
      ...
      if (!quote.audioSrc) {
        ...
        await generateImageWithPrompt(quote.text, filePath);
      }
    }
  }
}
